...
# Enhanced version with multi-email support, directory-based switching, and more

# Load core functionality
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/gh-switch-core.sh"

# Function to check prerequisites (Linux specific)
check_prerequisites() {
    # Check for git
...
# Function to switch to a profile (Linux-specific implementation)
switch_profile() {
    if [ $# -lt 1 ]; then
...
EOF
    ...
    if [ "$switch_type" != "auto" ]; then
        # Linux-specific: Update SSH config if needed
        ...
        if [ -n "$cred_helper" ]; then
            ...
            if [[ "$cred_helper" == *"libsecret"* ]]; then
                echo -e "${YELLOW}You're using libsecret for Git credentials.${NC}"
                echo "To manage stored credentials:"
                echo "1. Use 'secret-tool search protocol https host github.com' to find stored credentials"
                echo "2. Use 'secret-tool clear protocol https host github.com' to clear them"
                echo "3. Or use Seahorse (GUI) if available"
            ...
            echo "Or for better security with libsecret:"
            echo "git config --global credential.helper libsecret"
        fi
        
        # SSH agent tips for Linux
        echo -e "${YELLOW}Tip: Add your SSH key to ssh-agent with:${NC}"
