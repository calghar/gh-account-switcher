...
# Function to validate GPG key format
validate_gpg_key() {
    local key="$1"
...
# Function to migrate old profile format to new multi-email format
migrate_profiles() {
...
# Function to add email to existing profile
...
# Function to remove email from profile
...
# Function to list emails for a profile
list_profile_emails() {
    if [ $# -lt 1 ]; then
...
# Function to set git name for profile
set_profile_name() {
    if [ $# -lt 2 ]; then
        echo -e "${RED}Error: Missing arguments${NC}"
...
# Function to get current directory profile rule
...
# Function to add directory rule
add_directory_rule() {
    if [ $# -lt 2 ]; then
        echo -e "${RED}Error: Missing arguments${NC}"
...
# Function to remove directory rule
...
# Function to list directory rules
...
# Function to check and auto-switch profile based on directory
auto_switch_check() {
